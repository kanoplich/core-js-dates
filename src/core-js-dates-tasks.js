...
function getNextFriday(date) {
  ...
  const result = date.setDate(date.getDate() + count);
...
function getNextFridayThe13th(date) {
  ...
  return getNextFridayThe13th(newDate.setMonth(newDate.getMonth() + 1));
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (end >= start) {
    for (let i = 0; i < countWorkDays; i += 1) {
      result.push(start.toLocaleDateString().replace(/\./g, '-'));
